...
fields used depend on the $style value.
...
=item get_depends
	...
	my $lol_struct = $self->get_depends($run_or_build, $dep_or_confl)

Get the dependency (or conflicts) package list. If $run_or_build is
...
=item match_package
  ...
  my $result = Fink::PkgVersion::match_package($pkgspec);
  my $result = Fink::PkgVersion::match_package($pkgspec, $quiet);

Find a PkgVersion by matching a specification. Return undef
on failure. If $quiet is true, don't print messages.
...
real packages can be removed, not "Provides" virtuals.
...
# defaults for LD_PREBIND and LD_PREBIND_ALLOW_OVERLAP depend on the distribution
