...
sed "s|@VERSION@|$version|g ; s|@PREFIX@|$basepath|g" <fink.8.in \
  | perl -MTime::Local -MPOSIX=strftime -p -e 'if (s/(\.Dd \$Date$/\1/) {$epochtime = timegm($7,$6,$5,$4,$3-1,$2-1900);$datestr = strftime "%B %e, %Y", localtime($epochtime); s/(\.Dd )\$Date$1$datestr/;}' \
  >fink.8
