# Official mirror list: http://www.gnu.org/prep/ftp.html
Timestamp: 2010-10-01

Primary: ftp://ftp.gnu.org/gnu

afr-NA: ftp://download.polytechnic.edu.na/pub/ftp.gnu.org/gnu
afr-NA: http://download.polytechnic.edu.na/pub/ftp.gnu.org/gnu
afr-ZA: ftp://ftp.is.co.za/mirror/ftp.gnu.org/gnu
asi-CN: http://mirror.bjtu.edu.cn/gnu
asi-JP: http://core.ring.gr.jp/pub/GNU
asi-TW: ftp://ftp.ntu.edu.tw/pub/gnu/gnu
asi-TW: ftp://ftp.twaren.net/Unix/GNU/gnu
asi-TW: http://ftp.twaren.net/Unix/GNU/gnu
asi-VN: http://mirror-fpt-telecom.fpt.net/gnu
eur-AT: ftp://gd.tuwien.ac.at/gnu/gnusrc
eur-AT: http://gd.tuwien.ac.at/gnu/gnusrc
eur-BE: ftp://ftp.easynet.be/gnu
eur-BE: http://ftp.easynet.be/ftp/gnu
eur-CH: ftp://sunsite.cnlab-switch.ch/mirror/gnu
eur-CZ: ftp://ftp.sh.cvut.cz/MIRRORS/gnu/pub/gnu
eur-CZ: http://ftp.sh.cvut.cz/MIRRORS/gnu/pub/gnu
eur-DE: ftp://ftp-stud.fht-esslingen.de/pub/Mirrors/ftp.gnu.org
eur-DE: ftp://ftp.cs.tu-berlin.de/pub/gnu
eur-DE: ftp://ftp.cw.net/pub/gnu
eur-DE: ftp://ftp.halifax.rwth-aachen.de/gnu
eur-DE: ftp://ftp.informatik.rwth-aachen.de/pub/gnu
eur-DE: ftp://ftp.u-tx.net/gnu
eur-DE: http://artfiles.org/gnu.org/ftp/gnu
eur-DE: http://ftp.cw.net/pub/gnu
eur-DE: http://ftp.halifax.rwth-aachen.de/gnu
eur-DE: http://ftp.hosteurope.de/mirror/ftp.gnu.org/gnu
eur-DE: http://ftp.u-tx.net/gnu
eur-DK: ftp://mirrors.dotsrc.org/gnu
eur-DK: http://ftp.download-by.net/gnu/gnu
eur-DK: http://mirrors.dotsrc.org/gnu
eur-FI: ftp://ftp.funet.fi/pub/gnu/prep
eur-FI: http://www.nic.funet.fi/pub/gnu/ftp.gnu.org/pub/gnu
eur-FR: ftp://ftp.igh.cnrs.fr/pub/gnu
eur-FR: ftp://mirror.cict.fr/gnu
eur-FR: http://ftp.igh.cnrs.fr/pub/gnu
eur-FR: http://mirror.ibcp.fr/pub/gnu
eur-GR: ftp://ftp.cc.uoc.gr/mirrors/gnu
eur-GR: ftp://ftp.ntua.gr/pub/gnu
eur-GR: http://ftp.cc.uoc.gr/mirrors/gnu
eur-IE: ftp://ftp.esat.net/pub/gnu
eur-IE: ftp://ftp.heanet.ie/mirrors/ftp.gnu.org/gnu
eur-IE: http://ftp.heanet.ie/mirrors/ftp.gnu.org/gnu
eur-NL: ftp://ftp.mirror.nl/pub/mirror/gnu
eur-NL: ftp://ftp.nluug.nl/pub/gnu
eur-NO: ftp://ftp.uninett.no/pub/gnu
eur-NO: ftp://gnuftp.uib.no/pub/gnuftp
eur-NO: http://gnuftp.uib.no
eur-PL: ftp://ftp.task.gda.pl/pub/gnu
eur-PL: ftp://ftp.tpnet.pl/d12/gnu
eur-PL: ftp://sunsite.icm.edu.pl/pub/gnu
eur-PL: http://ftp.tpnet.pl/vol/d12/gnu
eur-PT: ftp://mirrors.fe.up.pt/pub/gnu
eur-PT: ftp://mirrors.nfsi.pt/pub/gnu
eur-PT: http://mirrors.fe.up.pt/pub/gnu
eur-PT: http://mirrors.nfsi.pt/gnu
eur-RU: ftp://ftp.chg.ru/pub/gnu
eur-SE: ftp://ftp.df.lth.se/pub/ftp.gnu.org/pub/gnu
eur-SE: ftp://ftp.isy.liu.se/pub/gnu
eur-SE: ftp://ftp.sunet.se/pub/gnu
eur-SE: ftp://mirrors.kernel.org/gnu
eur-SE: http://ftp.df.lth.se/pub/ftp.gnu.org/pub/gnu
eur-SE: http://mirrors.kernel.org/gnu
eur-SI: http://mirror.lihnidos.org/GNU/ftp/gnu
eur-TR: http://mirror.veriportal.com/gnu
eur-UA: ftp://ftp.gnu.org.ua/gnu
eur-UK: ftp://www.mirrorservice.org/sites/ftp.gnu.org/gnu
eur-UK: http://gnu.xl-mirror.nl
eur-UK: http://www.mirrorservice.org/sites/ftp.gnu.org/gnu
nam-CA: ftp://gnu.mirror.iweb.com/gnu
nam-CA: http://ftpmirror.gnu.org
nam-CA: http://gnu.mirror.iweb.com/gnu
nam-US: ftp://aeneas.mit.edu/pub/gnu
nam-US: ftp://ftp.club.cc.cmu.edu/gnu
nam-US: ftp://ftp.gnu.org/gnu
nam-US: ftp://gcondra.cs.washington.edu/gnu
nam-US: ftp://mirror.anl.gov/pub/gnu
nam-US: ftp://mirror.its.uidaho.edu/gnu
nam-US: ftp://mirrors.ecvps.com/gnu
nam-US: ftp://mirrors.usc.edu/pub/gnu
nam-US: http://astromirror.uchicago.edu/gnu
nam-US: http://devlinux.net/software/gnu
nam-US: http://ftp.club.cc.cmu.edu/pub/gnu
nam-US: http://ftp.wayne.edu/pub/gnu
nam-US: http://gnu.spinellicreations.com/gnu
nam-US: http://mirror.anl.gov/pub/gnu
nam-US: http://mirror.clarkson.edu/gnu
nam-US: http://mirror.its.uidaho.edu/pub/gnu
nam-US: http://mirrors.ecvps.com/gnu
nam-US: http://mirrors.ibiblio.org/pub/mirrors/gnu/ftp/gnu
nam-US: http://mirrors.usc.edu/pub/gnu
sam-BR: ftp://ftp.unicamp.br/pub/gnu
sam-BR: http://gnu.c3sl.ufpr.br/ftp
sam-CR: ftp://mirrors.ucr.ac.cr/GNU/gnu
sam-CR: http://mirrors.ucr.ac.cr/GNU/gnu
