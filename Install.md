 Fink 0.36.4.1 Installation
=========================

These are the installation instructions for the "source" distribution based on "fink-0.36.4.1" , intended for use with OS X v.10.6 and later.
...
------------------------------------
Start out by copying the "fink-0.36.4.1.tar.gz" file to your home folder (it might also show up as "fink-0.36.4.1.tar" if you used Safari to download it). Then, open Terminal.app and follow the session below. Computer output is in "`normal (monospaced) face`", your input is in **"bold face"** (or otherwise highlighted). The actual input prompts from the shell may vary, and some chunks of the output have been omitted ( "`...`" ).
...
If you want to do so, go ahead. You won't have to stop the Fink install to do that.
    ...
    [frodo:~] testuser% tar xf fink-0.36.4.1.tar.gz
    [frodo:~] testuser% cd fink-0.36.4.1
    [frodo:~/fink-0.36.4.1] testuser% ./bootstrap
...
**(press return)**
   ...
   **(take a coffee break while Fink downloads and compiles the base packages)**
   ...
   You should now have a working Fink installation in '/sw'.
    ...
    [frodo:~/fink-0.36.4.1] testuser% cd
    [frodo:~] testuser% rm -r fink-0.36.4.1
...
Installation
--------------
First, you need to unpack the fink-0.36.4.1.tar.gz tarball (it might also show up as "fink-0.36.4.1.tar" if you used Safari to download it). So, in a terminal window, go to the directory where you put the tarball, and run this command:

    tar xf fink-0.36.4.1.tar.gz

You now have a directory named fink-0.36.4.1. Change to it with "`cd fink-0.36.4.1`".

The actual installation is performed by the perl script `bootstrap`. So, to start installation, go to the fink-0.36.4.1 directory and run this command:
